  ...
  result.innerHTML = `
    <div class="ssm-result-card">
      <div class="ssm-result-top">
        ...
        <div class="ssm-price">Loading…</div>
      </div>
      <div class="ssm-note">Fetching latest available market quote.</div>
    </div>`;

  const url = `https://api.allorigins.win/raw?url=${encodeURIComponent('https://query1.finance.yahoo.com/v7/finance/quote?symbols=' + yahoo)}`;
  fetch(url)
    .then(resp => resp.ok ? resp.json() : Promise.reject(new Error('quote failed')))
    .then(data => {
      ...
      result.innerHTML = `
        <div class="ssm-result-card">
          <div class="ssm-result-top">
            ...
            <div class="ssm-price">${price ? price.toLocaleString('en-IN', { maximumFractionDigits: 2 }) : 'N/A'}</div>
          </div>
          <div class="ssm-change ${isPos ? 'pos' : 'neg'}">${isPos ? '▲' : '▼'} ${Math.abs(chg).toFixed(2)}%</div>
          <div class="ssm-note">Delayed quote shown from the available market feed.</div>
        </div>`;
    })
    .catch(() => {
      result.innerHTML = `
