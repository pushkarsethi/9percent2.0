...
function closeStockSearch() {
  ...
  const sug = el('ssmSuggest'); if (sug) { sug.style.display='none'; sug.innerHTML=''; }
  const res = el('ssmResult'); if (res) { res.style.display='none'; res.innerHTML=''; }
}

/* CORS proxies — used for both Yahoo search and quote */
const SSM_PROXIES = [
  u => `https://corsproxy.io/?${encodeURIComponent(u)}`,
  u => `https://api.allorigins.win/raw?url=${encodeURIComponent(u)}`,
  u => `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(u)}`,
];
async function ssmFetchJSON(target) {
  for (const wrap of SSM_PROXIES) {
    try {
      const r = await fetch(wrap(target), { signal: AbortSignal.timeout(7000) });
      if (!r.ok) continue;
      return await r.json();
    } catch (_) { /* next */ }
  }
...
let ssmSearchToken = 0;
let ssmDebounce = null;

function handleStockSearch(value) {
  const query = (value || '').trim();
  const chips = el('ssmChips');
  const sug = el('ssmSuggest');
  const result = el('ssmResult');
  if (chips) {
    const upper = query.toUpperCase();
    chips.querySelectorAll('.ssm-chip').forEach(chip => {
      chip.style.display = query ? (chip.textContent.toUpperCase().includes(upper) ? '' : 'none') : '';
    });
  }
  if (!query) {
  ...
  if (sug) {
    ...
    sug.innerHTML = `<div class="ssm-sug-loading"><div class="ssm-spinner"></div><span>Searching <strong>${query}</strong>…</span></div>`;
  }
  clearTimeout(ssmDebounce);
  const token = ++ssmSearchToken;
  ssmDebounce = setTimeout(async () => {
    const url = `https://query1.finance.yahoo.com/v1/finance/search?q=${encodeURIComponent(query)}&quotesCount=10&newsCount=0&listsCount=0&enableFuzzyQuery=true`;
    const data = await ssmFetchJSON(url);
    if (token !== ssmSearchToken) return;
    const items = (data?.quotes || []).filter(q => q.symbol).slice(0, 8);
    if (!sug) return;
    if (items.length === 0) {
      // Local fallback
      const local = (window.FOLIO_SYMBOLS || []).filter(s =>
        s.sym.toUpperCase().includes(query.toUpperCase()) || s.yahoo.toUpperCase().includes(query.toUpperCase())
      ).slice(0, 6);
      if (local.length === 0) {
        sug.innerHTML = `<div class="ssm-sug-empty">No matches for <strong>${query}</strong>. Try a different ticker.</div>`;
        return;
      }
      sug.innerHTML = local.map(l =>
        `<button class="ssm-sug-row" onclick="searchStock('${l.sym.replace(/'/g,"\\'")}','${l.yahoo}')">
           <span class="ssm-sug-sym">${l.yahoo}</span>
           <span class="ssm-sug-name">${l.sym}</span>
         </button>`
      ).join('');
      return;
    }
    sug.innerHTML = items.map(q => {
      const name = (q.shortname || q.longname || q.symbol).replace(/'/g, "\\'");
      const exch = q.exchange ? `<span class="ssm-sug-exch">${q.exchange}</span>` : '';
      const type = q.quoteType ? `<span class="ssm-sug-type">${q.quoteType}</span>` : '';
      return `<button class="ssm-sug-row" onclick="searchStock('${name}','${q.symbol}')">
        <span class="ssm-sug-sym">${q.symbol}</span>
        <span class="ssm-sug-name">${q.shortname || q.longname || ''}</span>
        <span class="ssm-sug-meta">${type}${exch}</span>
      </button>`;
    }).join('');
  }, 220);
}
...
function handleSearchKey(event) {
  if (event.key === 'Escape') return closeStockSearch();
  if (event.key !== 'Enter') return;
  const sug = el('ssmSuggest');
  const first = sug && sug.querySelector('.ssm-sug-row');
